...
cc.Class({
    ...
    start () {
        wx.onMessage(data => {
            switch (data.message) {
                ...
                case 'UploadGameData' : {
                    //上传得分
                    //data: {message:'UploadGameData', score:999}
                    this.UploadGameData(data.score);
                }
            }
        });
    },
    ...
    //上传玩家游戏数据
    UploadGameData (score) {
        var maxScore = Math.max(score, GameData.instance.maxScore);
        if (maxScore > GameData.instance.maxScore)
        {
            GameData.instance.maxScore = maxScore;//记录最高分
    ...
    GetUserGameData () {
        wx.getUserCloudStorage({
            keyList : ["score"],//要获取数据的key列表
            success :   function(res){
                console.log("下载玩家游戏数据成功!");
                if (res.KVDataList.length > 0)
                {
                    GameData.instance.maxScore = res.KVDataList[0].value;
                }
            }
        });
    },
